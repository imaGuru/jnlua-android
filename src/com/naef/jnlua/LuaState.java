 ...
 * 
 * <p>
 * The class performs extensive checking on all arguments and its state. Specifically, the following exceptions are thrown under
 ...
 * <td>if a stack index refers to a non-valid stack location and the API does not explicitly specify that the stack index may be
 * non-valid</td>
 * </tr>
 * <tr>
 * <td>{@link java.lang.IllegalArgumentException}</td>
 * <td>if a stack index refers to an stack location with value type that is different from the value type explicitly specified in
 * the API</td>
 * </tr>
 * <tr>
 * <td>{@link java.lang.IllegalArgumentException}</td>
 * <td>if a count is negative or out of range and the API does not explicitly specify that the count may be negative or out of
 * range</td>
 * </tr>
 * <tr>
 ...
 * <td>if an error occurs running a <code>__gc</code> metamethod during garbage collection</td>
 * </tr>
 * <tr>
...
public class LuaState {
	...
	/** The <code>lua_State</code> pointer on the JNI side. <code>0</code> implies that this Lua state is closed. The field is
	 * modified exclusively on the JNI side and must not be touched on the Java side. */
	private long luaState;

	/** The <code>lua_State</code> pointer on the JNI side for the running coroutine. This field is modified exclusively on the JNI
	 * side and must not be touched on the Java side. */
	private long luaThread;

	/** The yield flag. This field is modified from both the JNI side and Java side and signals a pending yield. */
	...
	// -- Construction
	/** Creates a new instance. The class loader of this Lua state is set to the context class loader of the calling thread. The
	 * Java reflector and the converter are initialized with the default implementations.
	 * 
	 * @see #getClassLoader()
	...
	/** Performs a garbage collection operation. Please see the Lua Reference Manual for an explanation of the actions, arguments
	 * and return values.
	 * 
	 * @param what the operation to perform
	 * @param data the argument required by some operations
	...
	// -- Registration
	/** Opens the specified library in this Lua state. The library is pushed onto the stack.
	 * 
	...
	/** Registers a module and pushes the module on the stack. Optionally, a module can be registered globally. As of Lua 5.2,
	 * modules are <i>not</i> expected to set global variables anymore.
	 * 
	 * @param moduleName the module name
	 * @param namedJavaFunctions the Java functions of the module
	 * @param global whether to register the module globally */
	public synchronized void register (String moduleName, NamedJavaFunction[] namedJavaFunctions, boolean global) {
		check();
		/*
		 * The following code corresponds to luaL_requiref() and must be kept in sync. The original code cannot be called due to the
		 * necessity of pushing each C function with an individual closure.
		 */
	...
	// -- Load and dump
	/** Loads a Lua chunk from an input stream and pushes it on the stack as a function. Depending on the value of mode, the the Lua
	 * chunk can either be a pre-compiled binary chunk or a UTF-8 encoded text chunk.
	 * 
	 * @param inputStream the input stream
	 * @param chunkName the name of the chunk for use in error messages
	 * @param mode <code>"b"</code> to accept binary, <code>"t"</code> to accept text, or <code>"bt"</code> to accept both
	...
	// -- Call
	/** Calls a Lua function. The function to call and the specified number of arguments are on the stack. After the call, the
	 * specified number of returns values are on stack. If the number of return values has been specified as {@link #MULTRET}, the
	 * number of values on the stack corresponds the to number of values actually returned by the called function.
	 * 
	 * @param argCount the number of arguments
	 * @param returnCount the number of return values, or {@link #MULTRET} to accept all values returned by the function */
	 ...
	 * 
	 * <p>
	 * You cannot push <code>null</code> without conversion since <code>null</code> is not a Java object. The converter converts
	...
	/** Returns whether the value at the specified stack index is a function (either a C function, a Java function or a Lua
	 * function.)
	 * 
	 * <p>
	 * The stack index may be non-valid.
	...
	/** Returns whether the value at the specified stack index is convertible to a Java object of the specified type. The conversion
	 * is checked by the configured converter.
	 * 
	 * <p>
	 * The stack index may be non-valid.
	 * </p>
	 ...
	 * 
	 * <p>
	 * Note that the method does not perform conversion. If you want to check if a value <i>is convertible to</i> a Java object,
	 * then invoke <code>
	...
	/** Returns whether the specified stack index is non-valid or its value is <code>nil</code>.
	 * 
	 * <p>
	 * The stack index may be non-valid.
	 * </p>
	 * 
	 * @param index the stack index
	 * @return whether the stack index is non-valid or its value is <code>nil</code> */
	...
	// -- Stack query
	/** Compares the values at two specified stack indexes for the specified operator according to Lua semantics.
	 * 
	 * <p>
	 * Any stack index may be non-valid in which case the method returns <code>false</code>.
	 * </p>
	 * 
	...
	/** Returns the length of the value at the specified stack index. Please see the Lua Reference Manual for the definition of the
	 * raw length of a value.
	 * 
	 * @param index the stack index
	 * @return the length
	...
	/** Bypassing metatable logic, returns the length of the value at the specified stack index. Please see the Lua Reference Manual
	 * for the definition of the raw length of a value.
	 * 
	 * @param index the stack index
	 * @return the length
	 * @since JNLua 1.0.0 */
	...
	/** Returns the boolean representation of the value at the specified stack index. The boolean representation is
	 * <code>true</code> for all values except <code>false</code> and <code>nil</code>. The method also returns <code>false</code>
	...
	/** Returns the byte array representation of the value at the specified stack index. The value must be a string or a number. If
	 * the value is a number, it is in place converted to a string. Otherwise, the method returns <code>null</code>.
	 * 
	 * @param index the stack index
	...
	/** Returns the integer representation of the value at the specified stack index. The value must be a number or a string
	 * convertible to a number. Otherwise, the method returns <code>0</code>.
	 * 
	 * @param index the stack index
	...
	/** Returns the integer representation of the value at the specified stack index. The value must be a number or a string
	 * convertible to a number. Otherwise, the method returns <code>null</code>.
	 * 
	 * @param index the stack index
	...
	/** Returns the Java function of the value at the specified stack index. If the value is not a Java function, the method returns
	 * <code>null</code>.
	 * 
	 * @param index the stack index
	...
	/** Returns a Java object of the specified type representing the value at the specified stack index. The value must be
	 * convertible to a Java object of the specified type. The conversion is executed by the configured converter.
	 * 
	 * @param index the stack index
	...
	/** Returns the Java object of the value at the specified stack index. If the value is not a Java object, the method returns
	 * <code>null</code>.
	 * 
	 * <p>
	 * Note that the method does not convert values to Java objects. If you require <i>any</i> Java object that represents the
	 * value at the specified index, then invoke <code>toJavaObject(index, Object.class)</code>.
	 * </p>
	 * 
	 * @param index the stack index
	...
	/** Returns the number representation of the value at the specified stack index. The value must be a number or a string
	 * convertible to a number. Otherwise, the method returns <code>0.0</code>.
	 * 
	 * @param index the stack index
	...
	/** Returns the pointer representation of the value at the specified stack index. The value must be a table, thread, function or
	 * userdata (such as a Java object.) Otherwise, the method returns <code>0L</code>. Different values return different pointers.
	...
	/** Returns the string representation of the value at the specified stack index. The value must be a string or a number. If the
	 * value is a number, it is in place converted to a string. Otherwise, the method returns <code>null</code>.
	 * 
	 * @param index the stack index
	...
	/** Returns the name of the type at the specified stack index. The type name is the display text for the Lua type except for
	 * Java objects where the type name is the canonical class name.
	 * 
	 * <p>
	 * The stack index may be non-valid in which case the method returns the string <code>"none"</code>.
	 * </p>
	 * 
	...
	/** Performs an arithmetic operation with values on top of the stack using Lua semantics.
	 * 
	...
	/** Copies a value at a specified index to another index, replacing the value at that index.
	 * 
	 * @param fromIndex the index to copy from
	 * @param toIndex the index to copy to
	 * @since JNLua 1.0.0 */
	...
	/** Pushes the length of the value at the specified stack index on the stack. The value pushed by the method corresponds to the
	 * Lua <code>#</code> operator.
	 * 
	 * @param index the index for which to push the length
	 * @since JNLua 1.0.0 */
	...
	// -- Table
	/** Pushes on the stack the value indexed by the key on top of the stack in the table at the specified index. The key is
	...
	/** Pops a key from the stack and pushes on the stack the next key and its value in the table at the specified index. If there
	 * is no next key, the key is popped but nothing is pushed. The method returns whether there is a next key.
	 * 
	 * @param index the stack index containing the table
	...
	/** Bypassing metatable logic, pushes on the stack the value indexed by the key on top of the stack in the table at the
	 * specified index. The key is replaced by the value from the table.
	 * 
	 * @param index the stack index containing the table */
	...
	/** Bypassing metatable logic, pushes on the stack the value indexed by the specified integer key in the table at the specified
	 * index.
	 * 
	 * @param index the stack index containing the table
	...
	/** Bypassing metatable logic, sets the value on top of the stack in the table at the specified index using the value on the
	 * second highest stack position as the key. Both the value and the key are popped from the stack.
	 * 
	 * @param index the stack index containing the table */
	...
	/** Bypassing metatable logic, sets the value on top of the stack in the table at the specified index using the specified
	 * integer key. The value is popped from the stack.
	 * 
	 * @param index the stack index containing the table
	 * @param key the integer key */
	...
	/** Sets the value on top of the stack in the table at the specified index using the value on the second highest stack position
	 * as the key. Both the value and the key are popped from the stack.
	 * 
	 * @param index the stack index containing the table */
	...
	/** Sets the value on top of the stack in the table at the specified index using the specified string key. The value is popped
	 * from the stack.
	 * 
	 * @param index the stack index containing the table
	 * @param key the string key */
	...
	// -- Metatable
	/** Pushes on the stack the value of the named field in the metatable of the value at the specified index and returns
	 * <code>true</code>. If the value does not have a metatable or if the metatable does not contain the named field, nothing is
	 * pushed and the method returns <code>false</code>.
	 * 
	...
	/** Pushes on the stack the metatable of the value at the specified index. If the value does not have a metatable, the method
	 * returns <code>false</code> and nothing is pushed.
	 * 
	...
	/** Sets the value on top of the stack as the metatable of the value at the specified index. The metatable to be set is popped
	 * from the stack regardless whether it can be set or not.
	 * 
	...
	// -- Thread
	/** Pops the start function of a new Lua thread from the stack and creates the new thread with that start function. The new
	 * thread is pushed on the stack. */
	public synchronized void newThread () {
		check();
		lua_newthread();
	}

	/** Resumes the thread at the specified stack index, popping the specified number of arguments from the top of the stack and
	 * passing them to the resumed thread. The method returns the number of values pushed on the stack as the return values of the
	 * resumed thread.
	 * 
	...
	/** Returns the status of the thread at the specified stack index. If the thread is in initial state of has finished its
	 * execution, the method returns <code>0</code>. If the thread has yielded, the method returns {@link #YIELD}. Other return
	 * values indicate errors for which an exception has been thrown.
	 * 
	 * @param index the index
	 * @return the status */
	...
	/** Yields the running thread, popping the specified number of values from the top of the stack and passing them as return
	 * values to the thread which has resumed the running thread. The method must be used exclusively at the exit point of Java
	 * functions, i.e. <code>return luaState.yield(n)</code>.
	 * 
	...
	// -- Reference
	/** Stores the value on top of the stack in the table at the specified index and returns the integer key of the value in that
	 * table as a reference. The value is popped from the stack.
	 * 
	...
	/** Removes a previously created reference from the table at the specified index. The value is removed from the table and its
	 * integer key of the reference is freed for reuse.
	 * 
	 * @param index the stack index containing the table where the value was stored
	 ...
	 * 
	 * <p>
	 * The method provides optimized performance over a Java implementation of the same functionality due to the reduced number of
	 * JNI transitions.
	 * </p>
	 * 
	 * @param index the stack index containing the table
	...
	// -- Argument checking
	/** Checks if a condition is true for the specified function argument. If not, the method throws a Lua runtime exception with
	...
	/** Checks if the value of the specified function argument is a string or a number. If so, the argument value is returned as a
	 * byte array. Otherwise, the method throws a Lua runtime exception with a descriptive error message.
	 * 
	 * @param index the argument index
	...
	/** Checks if the value of the specified function argument is a string or a number. If so, the argument value is returned as a
	 * byte array. If the value of the specified argument is undefined or <code>nil</code>, the method returns the specified
	 * default value. Otherwise, the method throws a Lua runtime exception with a descriptive error message.
	 * 
	 * @param index the argument index
	...
	/** Checks if the value of the specified function argument is a string or a number matching the name of one of the specified
	 * enum values. If so, the argument value is returned as an enum value. Otherwise, the method throws a Lua runtime exception
	...
	/** Checks if the value of the specified function argument is a string or a number matching one of the specified enum values. If
	 * so, the argument value is returned as an enum value. If the specified stack index is non-valid or if its value is
	 * <code>nil</code>, the method returns the specified default value. Otherwise, the method throws a Lua runtime exception with
	...
	public synchronized <T extends Enum<T>> T checkEnum (int index, T[] values, T d) {
		check();
		String s = d != null ? checkString(index, d.name()) : checkString(index);
	...
	/** Checks if the value of the specified function argument is a number or a string convertible to a number. If so, the argument
	 * value is returned as an integer. Otherwise, the method throws a Lua runtime exception with a descriptive error message.
	 * 
	 * @param index the argument index
	...
	/** Checks if the value of the specified function argument is a number or a string convertible to a number. If so, the argument
	 * value is returned as an integer. If the specified stack index is non-valid or if its value is <code>nil</code>, the method
	 * returns the specified default value. Otherwise, the method throws a Lua runtime exception with a descriptive error message.
	 * 
	 * @param index the argument index
	 * @param d the default value
	...
	/** Checks if the value of the specified function argument is convertible to a Java object of the specified type. If so, the
	 * argument value is returned as a Java object of the specified type. Otherwise, the method throws a Lua runtime exception with
	 * a descriptive error message.
	 * 
	 * <p>
	 * Note that the converter converts <code>nil</code> to <code>null</code>. Therefore, the method may return <code>null</code>
	 * if the value is <code>nil</code>.
	 * </p>
	 * 
	 * @param index the argument index
	...
	public synchronized <T> T checkJavaObject (int index, Class<T> clazz) {
		...
		if (!isJavaObject(index, clazz)) {
			checkArg(index, false, String.format("%s expected, got %s", clazz.getCanonicalName(), typeName(index)));
		}
		return toJavaObject(index, clazz);
	}

	/** Checks if the value of the specified function argument is convertible to a Java object of the specified type. If so, the
	 * argument value is returned as a Java object of the specified type. If the specified stack index is non-valid or if its value
	 * is <code>nil</code>, the method returns the specified default value. Otherwise, the method throws a Lua runtime exception
	 * with a descriptive error message.
	 * 
	 * @param index the argument index
	...
	/** Checks if the value of the specified function argument is a number or a string convertible to a number. If so, the argument
	 * value is returned as a number. Otherwise, the method throws a Lua runtime exception with a descriptive error message.
	 * 
	 * @param index the argument index
	...
	/** Checks if the value of the specified function argument is a number or a string convertible to a number. If so, the argument
	 * value is returned as a number. If the specified stack index is non-valid or if its value is <code>nil</code>, the method
	 * returns the specified default value. Otherwise, the method throws a Lua runtime exception with a descriptive error message.
	 * 
	 * @param index the argument index
	 * @param d the default value
	...
	/** Checks if the value of the specified function argument is a string or a number matching one of the specified options. If so,
	 * the index position of the matched option is returned. Otherwise, the method throws a Lua runtime exception with a
	...
	/** Checks if the value of the specified function argument is a string or a number matching one of the specified options. If so,
	 * the index position of the matched option is returned. If the specified stack index is non-valid or if its value is
	 * <code>nil</code>, the method matches the specified default value. If no match is found, the method throws a Lua runtime
	...
	/** Checks if the value of the specified function argument is a string or a number. If so, the argument value is returned as a
	 * string. Otherwise, the method throws a Lua runtime exception with a descriptive error message.
	 * 
	 * @param index the argument index
	 * @return the string value */
	...
	/** Checks if the value of the specified function argument is a string or a number. If so, the argument value is returned as a
	 * string. If the specified stack index is non-valid or if its value is <code>nil</code>, the method returns the specified
	 * default value. Otherwise, the method throws a Lua runtime exception with a descriptive error message.
	 * 
	 * @param index the argument index
	 * @param d the default value
	...
	/** Checks if the value of the specified function argument is of the specified type. If not, the method throws a Lua runtime
	 * exception with a descriptive error message.
	 * 
	 * @param index the argument index
	 * @param type the type */
	...
	/** Returns a proxy object implementing the specified interface in Lua. The table at the specified stack index contains the
	 * method names from the interface as keys and the Lua functions implementing the interface methods as values. The returned
	 * object always implements the {@link LuaValueProxy} interface in addition to the specified interface.
	 * 
	 * @param index the stack index containing the table
	...
	/** Returns a proxy object implementing the specified list of interfaces in Lua. The table at the specified stack index contains
	 * the method names from the interfaces as keys and the Lua functions implementing the interface methods as values. The
	 * returned object always implements the {@link LuaValueProxy} interface in addition to the specified interfaces.
	 * 
	 * @param index the stack index containing the table
	...
	private LuaRuntimeException getArgTypeException (int index, LuaType type) {
		return getArgException(index,
			String.format("%s expected, got %s", type.toString().toLowerCase(Locale.US), type(index).toString().toLowerCase()));
	}
	...
	private LuaRuntimeException getArgException (int index, String extraMsg) {
		...
		// Format message
		String msg;
		String argument = index > 0 ? String.format(Locale.US, "argument #%d", index) : "self argument";
		if (name != null) {
			msg = String.format("bad %s to '%s' (%s)", argument, name, extraMsg);
		} else {
	...
	/** Represents a Lua garbage collector action. Please see the Lua Reference Manual for an explanation of these actions. */
	...
	/** Represents a Lua arithmetic operator. Please see the Lua Reference Manual for an explanation of these operators.
	 * 
	 * @since JNLua 1.0.0 */
	...
	/** Represents a Lua relational operator. Please see the Lua Reference Manual for an explanation of these operators.
	 * 
	 * @since JNLua 1.0.0 */
	...
	private static class LuaDebug {
		/** The <code>lua_Debug</code> pointer on the JNI side. <code>0</code> implies that the activation record has been freed. The
		 * field is modified exclusively on the JNI side and must not be touched on the Java side. */
		...
		// -- Properties
		/** Returns a reasonable name for the function given by this activation record, or <code>null</code> if none is found. */
